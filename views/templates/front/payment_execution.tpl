...
{capture name=path}
    ...
    <span class="navigation-pipe">{$navigationPipe|escape:'html':'UTF-8'}</span>{l s='Pilibaba payment' mod='pilipay'}
{/capture}
                    ...
                    <div class="form-group">
                        ...
                        <select id="currency_payment" class="form-control" name="currency_payment">
                            {foreach from=$currencies item=currency}
                                <option value="{$currency.id_currency|escape:'html':'UTF-8'}" {if $currency.id_currency == $cust_currency}selected="selected"{/if}>
                                    {$currency.name|escape:'html':'UTF-8'}
                                </option>
                            {/foreach}
                        </select>
                    </div>
                {else}
                    {l s='We allow the following currency to be sent via Pilibaba:' mod='pilipay'}&nbsp;<b>{$currencies.0.name|escape:'html':'UTF-8'}</b>
                    <input type="hidden" name="currency_payment" value="{$currencies.0.id_currency|escape:'html':'UTF-8'}" />
                {/if}
            </p>
            <p>
